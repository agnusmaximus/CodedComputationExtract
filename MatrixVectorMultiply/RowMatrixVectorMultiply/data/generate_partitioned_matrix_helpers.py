...
def generate_random_matrix(n_rows, n_cols):
    return [[np.random.uniform(low=-10,high=10) for i in range(n_cols)] for x in range(n_rows)]

def generate_random_matrix_inplace(mat, n_rows, n_cols):
    for i in range(n_rows):
        for j in range(n_cols):
            #mat[i][j] = np.random.uniform(low=0,high=100)
...
def generate_random_vector(n):
    return [random.randint(-10, 10) for x in range(n)]
